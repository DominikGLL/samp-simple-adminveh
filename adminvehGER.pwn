...
COMMAND:adminveh(playerid, params[])
{
	if(AdminVeh[playerid] != INVALID_VEHICLE_ID)
	{
	    ...
	    return SendClientMessage(playerid, -1, "Adminfahrzeug gelöscht!");
	}
	...
	if(strval(params) < 400 || strval(params) > 611)return SendClientMessage(playerid, -1, "{FF0000}FEHLER: {FFFFFF}Ungültige Fahrzeug ID.");
...
COMMAND:adminvehlock(playerid, params[])
{
	...
	new Float:vehx, Float:vehy, Float:vehz, string[128];
 	...
 	if(!IsPlayerInRangeOfPoint(playerid, 5.0, vehx, vehy, vehz))return SendClientMessage(playerid, -1, "{FF0000}FEHLER: {FFFFFF}Dein Adminfahrzeug ist nicht in deiner Nähe!");
 	GetVehicleParamsEx(AdminVeh[playerid], engine, lights, alarm, doors, bonnet, boot, objective);
